...
def patch_apply(driver):
	...
	if not os.path.exists(patchdir) or not os.path.isdir(patchdir):
        ...
        wifidir = os.path.join(linuxdir, "drivers/net/wireless/mediatek/")
	cmd = "mkdir -p {0}".format(os.path.join(wifidir,"patches",driver))
	execute(cmd)
	for patch in sorted(os.listdir(patchdir)):
		# check if a patch is already applied
		_target = os.path.join(wifidir,"patches",driver, patch)
		...
		if os.path.isfile(patch_fullpath) and re.match(r".+\.patch", patch):
				...
				linuxdir=wifidir, quilt=quilt, patch=patch_fullpath)
		        execute(cmd)
                        print("execmd:",cmd)
			cmd="cd {linuxdir};{quilt} next".format(linuxdir=wifidir, quilt=quilt)
			fp = os.popen(cmd)
...
def init_drivers():
	...
	for driver in drivers:
		...
		if not re.match(r"mt76.{13}|mt_wifi", driver) or not os.path.exists(makefile):
			print ("match fail" ,driver)
			continue
                print("LabelDebug mtk driver match:",driver)
		found = 0
