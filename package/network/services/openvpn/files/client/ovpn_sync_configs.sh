...
CONFIG="/sbin/uci"
product="$(/usr/bin/envctl factory get model)"
remote_location="https://http.fw.updates1.netgear.com/sw-apps/vpn-client/r7800"
vpn_country=`$CONFIG get openvpn.vpn_client.vpn_client_ovpn_cfg_country 2> /dev/null`
vpn_city=`$CONFIG get openvpn.vpn_client.vpn_client_ovpn_cfg_city 2> /dev/null`
vpn_protocol=`$CONFIG get openvpn.vpn_client.vpn_client_ovpn_cfg_protocol 2> /dev/null`
vpn_provider=`$CONFIG get openvpn.vpn_client.vpn_client_ovpn_cfg_provider 2> /dev/null`

id_file="updated_date.txt"

curl_opt="-s --capath /etc/ssl/certs --retry 5"
...
download_and_expand_configs()
{
	...
	if [ "x`$CONFIG get openvpn.vpn_client.vpn_client_ovpn_cfg_city 2> /dev/null`" = "xAny City" ]; then
...
while true; do
	if [ -f ${ovpn_client_cfg_dir}/${providerlist_file_name}.bak ]; then
		mv ${ovpn_client_cfg_dir}/${providerlist_file_name}.bak ${ovpn_client_cfg_dir}/${providerlist_file_name}.new 2>&-
	else
	/usr/bin/curl ${curl_opt} ${remote_location}/providerlist20180914.json -o ${ovpn_client_cfg_dir}/${providerlist_file_name}.new
	fi
	...
	if [ ! -f ${ovpn_client_cfg_dir}/${providerlist_file_name} ] \
		...
		mv ${ovpn_client_cfg_dir}/${providerlist_file_name}.new ${ovpn_client_cfg_dir}/${providerlist_file_name} 2>&-
