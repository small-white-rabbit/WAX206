...
router_region=`envctl factory get region`
...
case "$(/sbin/uci get openvpn.vpn_client.vpn_client_enabled 2> /dev/null)" in
...
{
	server_isp="${server_isp:=$(/sbin/uci get openvpn.vpn_client.vpn_client_ovpn_cfg_provider 2> /dev/null)}"
	case $server_isp in
		"PureVPN")
...
{
	server_country="${server_country:=$(/sbin/uci get openvpn.vpn_client.vpn_client_ovpn_cfg_country 2> /dev/null)}"
	server_city="${server_city:=$(/sbin/uci get openvpn.vpn_client.vpn_client_ovpn_cfg_city 2> /dev/null)}"
	server_user="${server_user:=$(/sbin/uci get openvpn.vpn_client.vpn_client_username 2> /dev/null)}"
	server_pass="${server_pass:=$(/sbin/uci get openvpn.vpn_client.vpn_client_password 2> /dev/null)}"
...
case $sub_cmd in
	...
	"enable")
		...
		if [ "$vpn_client_enable" = "on" ]; then
			#In DUT boot up, vpn client will be bring up by default.script&ip-up&net-wan, add this code to void double calling
			if [ -f "/tmp/boot_status" ]; then
				echo "openvpn_client.sh: will not start in boot sequense since default.script&ip-up&net-wan will bring up vpn client"
			else
				get_and_check_isp_of_ovpn
				get_and_check_detail_args_of_ovpn
