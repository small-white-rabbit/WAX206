...
function character_escape(str)
    local s,n = string.gsub(str, "[`\\$\"]", function(c) return "\\"..c end)
    return s
end
...
function __set_wifi_apcli_security(cfgs, diff, device, devname)
    ...
    local ssid_new = diff.ApCliSsid1 and diff.ApCliSsid1[2]:split() or ssid_old
    local ssid_new_i = (ssid_new[1] or ''):split(";") 
    ...
    os.execute("echo [quick_settings] start __set_wifi_apcli_security >> /tmp/wifiLuaDebug.log")
    for i = 1, num do
        local changed = false
        ...
        elseif next(keyid_new) and keyid_old_i[i] ~= keyid_new_i[i] then
            changed = true
        elseif next(ssid_new) and ssid_old_i[i] ~= ssid_new_i[i] then
            changed = true
        ...
        if changed then
            ...
            vif.WPAPSK = character_escape(diff["ApCliWPAPSK"] and diff["ApCliWPAPSK"][2] or cfgs["ApCliWPAPSK"])
            vif.SSID = character_escape(diff["ApCliSsid1"] and diff["ApCliSsid1"][2] or cfgs["ApCliSsid1"])
            vif.ApCliEnable = diff["ApCliEnable"] and diff["ApCliEnable"][2] or cfgs["ApCliEnable"]
    ...
    for i, vif in ipairs(vifs) do
        commands = string.format([[
                iwpriv %s set ApCliEnable="0";]], vif.vifname)
        ...
        elseif vif.AuthMode == "OWE" then
            commands = string.format([[
                ...
                iwpriv %s set ApCliPMFMFPR=1;
                iwpriv %s set ApCliPMFSHA256=1;
                iwpriv %s set IEEE8021X=0;]],
            vif.vifname, vif.vifname, vif.vifname, vif.vifname, vif.vifname, vif.vifname)
        elseif vif.AuthMode == "SHARED" then
            commands = string.format([[
                ...
                iwpriv %s set IEEE8021X=0;]],
            vif.vifname, vif.vifname, vif.vifname,  vif.DefaultKeyID_idx,
            vif.DefaultKey, vif.vifname, vif.KeyID, vif.vifname, vif.vifname, vif.vifname, vif.vifname)
        elseif vif.AuthMode == "WPA2PSK" then
            ...
            vif.vifname, vif.vifname, vif.EncrypType, vif.vifname, vif.vifname, vif.vifname, vif.vifname, vif.WPAPSK)
        ...
        elseif vif.AuthMode == "WPAPSKWPA2PSK" then
            commands = string.format([[
                ...
                iwpriv %s set ApCliPMFSHA256=0;
                iwpriv %s set ApCliWPAPSK="%s";]],
            vif.vifname, vif.vifname, vif.EncrypType, vif.vifname, vif.vifname, vif.vifname, vif.vifname, vif.vifname, vif.WPAPSK)
        ...
        commands = commands .."\n".. string.format([[
                iwpriv %s set ApCliSSID="%s";
                iwpriv %s set ApCliEnable="%s";]], vif.vifname, vif.SSID, vif.vifname, vif.ApCliEnable)
        debug_info_write(devname, commands)
        os.execute("touch /tmp/mtk/wifi/"..vif.vifname..".changed")
...
function __set_wifi_security(cfgs, diff, device, devname)
    ...
    local wscmode_old = cfgs.WscConfMode:split()
    ...
    os.execute("echo [quick_settings] start __set_wifi_security >> /tmp/wifiLuaDebug.log")
    ...
    if diff.WscConfMode then
        wscmode_new = diff.WscConfMode[2]:split()
        wscmode_new1 = wscmode_new[1]:split(";")
    end
    -- For WPA/WPA2
    ...
    local wscmode_old1 = wscmode_old[1]:split(";")
    ...
    for i = 1, #encr_old1 do
        ...
        elseif diff["RADIUS_Key"..tostring(i)] then
            changed = true
        elseif next(wscmode_new) and wscmode_old1[i] ~= wscmode_new1[i] then
            changed = true
        else
            for j = 1, 4 do
        ...
        if changed then
            ...
            vif.WEPTypeVal = diff["WEP"..tostring(vif.KeyID).."Type"..tostring(i)] and
                        diff["WEP"..tostring(vif.KeyID).."Type"..tostring(i)][2] or cfgs["WEP"..tostring(vif.KeyID).."Type"..tostring(i)]
            vif.WPAPSK = character_escape(diff["WPAPSK"..tostring(i)] and diff["WPAPSK"..tostring(i)][2] or cfgs["WPAPSK"..tostring(i)])
            vif.SSID = character_escape(diff["SSID"..tostring(i)] and diff["SSID"..tostring(i)][2] or cfgs["SSID"..tostring(i)])
            ...
            vif.RADIUS_Key = character_escape(diff["RADIUS_Key"..tostring(i)] and diff["RADIUS_Key"..tostring(i)][2] or cfgs["RADIUS_Key"..tostring(i)])
            vif.WscConfMode = wscmode_new1 and wscmode_new1[i] or wscmode_old1[i]
            table.insert(vifs, vif)
        end
    end
    ...
    for i, vif in ipairs(vifs) do
        ...
        commands = commands .."\n".. string.format([[
                iwpriv %s set WscConfMode="%s";]], vif.vifname, vif.WscConfMode)
        -- must append extra SSID command to make changes take effect
        commands = commands .."\n".. string.format([[
...
function __set_wifi_misc(cfgs, diff, device,devname)
    ...
    local last_command = string.format([[
                iwpriv %s set SSID="%s";]], vifname, character_escape(cfgs.SSID1))

    local vifidx = cfgs.AuthMode:split(";")
    local bssid_num = cfgs["BssidNum"]
    ...
    os.execute("echo [quick_settings] start __set_wifi_misc >> /tmp/wifiLuaDebug.log")
    os.execute("echo [quick_settings] vifname = "..vifname.."  vifext = "..vifext.." >> /tmp/wifiLuaDebug.log")
    os.execute("echo [quick_settings] bssid_num = "..bssid_num.."  >> /tmp/wifiLuaDebug.log")
    for k,v in pairs(diff) do
        local commands, commands_1, commands_2, commandns
        local commands_vifs, val
        
        os.execute("echo [quick_settings] key = "..k.." >> /tmp/wifiLuaDebug.log")
        if k:find("^SSID") then
            local _,_,i = string.find(k, "^SSID([%d]+)")
            if i == "1" then
                last_command = string.format([[
                iwpriv %s set SSID="%s";]], vifname, character_escape(tostring(v[2])))
                commands_dev = true
            else
                commands_ssid[tonumber(i)] = string.format([[
                iwpriv %s set SSID="%s";]], vifext..tostring(tonumber(i)-1), character_escape(tostring(v[2])))
            end
        ----------------------------------------------------------------------------------------------------
                    -----------------------------device config ----------------------------
        ...
        elseif match_vif_parm(k) then
            for i=1, #vifidx  do
              if token(cfgs[k], i) ~= token(diff[k][2], i) then
                subparam = token(diff[k][2], i)
                if subparam == nil then
                    subparam = diff[k][2]
                end
                commands_vifs = string.format([[
                    iwpriv %s set %s=%s;]], vifext..tostring(tonumber(i)-1), tostring(match_vif_parm(k)), subparam)
                commands_ssid[i] = string.format([[
                    iwpriv %s set SSID="%s";]], vifext..tostring(tonumber(i)-1), character_escape(diff["SSID"..tostring(i)] and tostring(diff["SSID"..tostring(i)][2]) or cfgs["SSID"..tostring(i)]))
        ...
        elseif match_vif_parm_no_ssid(k) then
            for i=1, #vifidx  do
              if token(cfgs[k], i) ~= token(diff[k][2], i) then
                ...
                commands_vifs = string.format([[
                    iwpriv %s set %s=%s;]], vifext..tostring(tonumber(i)-1), tostring(match_vif_parm_no_ssid(k)), subparam)
                    debug_info_write(devname, commands_vifs)
              end
            end

        elseif k == "WirelessMode" then 
            local bssid_str = tonumber(bssid_num)
            os.execute("echo [quick_settings] WirelessMode  bssid_str = "..bssid_str.." >> /tmp/wifiLuaDebug.log")
            for i=1, bssid_str  do
               commands_vifs = string.format([[
               iwpriv %s set WirelessMode=%s;]], vifext..tostring(tonumber(i)-1), tostring(v[2]))
               debug_info_write(devname, commands_vifs)
               os.execute("echo [quick_settings] WirelessMode devname = "..devname.." ifname = "..vifext..tostring(tonumber(i)-1).." >> /tmp/wifiLuaDebug.log")
            end
        ...
        elseif k == "RekeyInterval" or k == "rekeyinterval" then
            for i=1, #vifidx  do
              if token(cfgs.RekeyInterval, i) ~= token(diff.RekeyInterval[2], i) then
                subparam = token(diff.RekeyInterval[2], i)
                if subparam == nil then
                    subparam = diff.RekeyInterval[2]
                end
                commands_vifs = string.format([[
                iwpriv %s set RekeyInterval=%s;]], vifext..tostring(tonumber(i)-1), subparam)
        ...
        elseif not commands_bw and k == "HT_BSSCoexistence" then
            for i=1, #vifidx  do
                ...
                iwpriv %s set HtBw=%s;]], vifext..tostring(tonumber(i)-1), string.sub(diff["HT_BW"] and tostring(diff["HT_BW"][2])
                    or tostring(cfgs["HT_BW"]),1 ,1))
                commands_2 = string.format([[
                iwpriv %s set VhtBw=%s;]], vifext..tostring(tonumber(i)-1), string.sub(diff["VHT_BW"] and tostring(diff["VHT_BW"][2])
                    or tostring(cfgs["VHT_BW"]),1 ,1))
        ...
        elseif not commands_bw and k == "HT_BW" then
            ...
            for i=1, #vifidx  do
                commands_vifs = string.format([[
                iwpriv %s set HtBw=%s;]], vifext..tostring(tonumber(i)-1), string.sub(tostring(v[2]), 1, 1))
                commands_1 = string.format([[
                iwpriv %s set VhtBw=%s;]], vifext..tostring(tonumber(i)-1), string.sub(tostring(vhtbw), 1, 1))
                -- workaround
            ...
            -- add for apcli device when apcli device up/down maybe change the VT modify by T&W 20200826
            for i=1, 1  do
                commands_vifs = string.format([[
                iwpriv %s set HtBw=%s;]], vifapcli..tostring(tonumber(i)-1), string.sub(tostring(v[2]), 1, 1))
                commands_1 = string.format([[
                iwpriv %s set VhtBw=%s;]], vifapcli..tostring(tonumber(i)-1), string.sub(tostring(vhtbw), 1, 1))
                -- workaround
                if htbw == "1" and vhtbw == "0" then
                    commands_2 = string.format([[
                iwpriv %s set HtBssCoex=%s;]], vifapcli..tostring(tonumber(i)-1), diff["HT_BSSCoexistence"]
                                and tostring(diff["HT_BSSCoexistence"][2]) or tostring(cfgs["HT_BSSCoexistence"]))
                else
                    commands_2 = string.format([[
                iwpriv %s set HtBssCoex=0;]], vifapcli..tostring(tonumber(i)-1))
                end
                if commands_vifs then
                    debug_info_write(devname, commands_vifs)
                    debug_info_write(devname, commands_1)
        ...
        elseif not commands_bw and k == "VHT_BW" then
            ...
            for i=1, #vifidx  do
                commands_vifs = string.format([[
                iwpriv %s set VhtBw=%s;]], vifext..tostring(tonumber(i)-1), string.sub(tostring(v[2]), 1, 1))
                commands_1 = string.format([[
                iwpriv %s set HtBw=%s;]], vifext..tostring(tonumber(i)-1), string.sub(tostring(htbw)
                    or tostring(cfgs["HT_BW"]), 1, 1))
                 -- workaround
                if htbw == "1" and vhtbw == "0" then
                    commands_2 = string.format([[
            ...
            -- add for apcli device when apcli device up/down maybe change the VTH modify by T&W 20200826
            for i=1, 1  do
                commands_vifs = string.format([[
                iwpriv %s set VhtBw=%s;]], vifapcli..tostring(tonumber(i)-1), string.sub(tostring(v[2]), 1, 1))
                commands_1 = string.format([[
                iwpriv %s set HtBw=%s;]], vifapcli..tostring(tonumber(i)-1), string.sub(tostring(htbw)
                    or tostring(cfgs["HT_BW"]), 1, 1))
                 -- workaround
                if htbw == "1" and vhtbw == "0" then
                    commands_2 = string.format([[
        ...
        elseif vifs_cfg_parm(k) and vifs_cfg_parm(v[2]) then
          if string.find(k, "ApCli") then
            --commands_1 = string.format([[
            --    iwpriv %s set %s="%s";]], vifapcli..tostring(0), k, character_escape(tostring(v[2])))
            --debug_info_write(devname, commands_1)
          else
            commands = string.format([[
...
function quick_settings(devname,path)
    ...
    os.execute("echo [quick_settings] path = "..path.." >> /tmp/wifiLuaDebug.log")
    ...
    os.execute("echo [quick_settings] devname = "..devname.." path = "..path.." >> /tmp/wifiLuaDebug.log")  
    if not next(diff) then return false end -- diff == nil
    os.execute("echo [quick_settings] config update >> /tmp/wifiLuaDebug.log")
    ...
    need_downup_parms = {"HT_LDPC", "VHT_SGI", "VHT_LDPC", "idle_timeout_interval",
                          ...
                          "ChannelGrp","TxOP", "MuOfdma", "MuMimo", "SREnable", "SRMode", "SREnable", "G_BAND_256QAM","RTSThreshold",
                          "MUTxRxEnable", "ETxBfEnCond","MuOfdmaDlEnable","MuOfdmaUlEnable", "Vht1024QamSupport", "TxPreamble",
                          "TxPower", "radio", "RadioOn", "CountryRegionABand", "CountryRegion"}
    ...
    for k, v in pairs(diff) do
        ...
        for _, pat in ipairs(need_downup_parms) do
            if string.find(k, pat) then
                ...
                os.execute("echo [quick_settings] need_downup key = "..k.."="..v[2].." >> /tmp/wifiLuaDebug.log")
                break
            end
        end
    ...
    if need_downup then return true end
    os.execute("echo [quick_settings] need_downup is false >> /tmp/wifiLuaDebug.log")
