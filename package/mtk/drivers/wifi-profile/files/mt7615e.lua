...
function d8021xd_setup(devname)
    local mtkwifi = require("mtkwifi")
    ...
    if not wifi_services_exist then
        return
    end
    ...
    if l1parser and devs then
        ...
        if cfgs.RadioOn ~= "1" then
            return
        end
        
        --for i,vif_enable in ipairs(vifidx) do
            --if vif_enable == "1" then
                --d8021xd_chk(devname, dev.ext_ifname, dev.ext_ifname..tostring(i-1), true)
            --end
        --end
        os.execute("echo [d8021xd_setup] enter d8021xd_chk >> /tmp/wifiLuaDebug.log")
...
function mt7615e_up(devname)
    ...
    local devs, l1parser = mtkwifi.__get_l1dat()
    local alldevs = mtkwifi.get_all_devs()
    ...
    if l1parser and devs then
        ...
        if mtkwifi.exists("/sys/class/net/"..dev.main_ifname) then
            ssid_index = alldevs[devname]["vifs"][dev.main_ifname].vifidx
            if vifidx[ssid_index] ~= "0" then
        ...
        do
            ...
            (  (string.match(vif, esc(dev.ext_ifname).."[0-9]+") and
                vifidx[alldevs[devname]["vifs"][vif].vifidx] ~= "0")
        ...
        if wifi_services_exist then
             os.execute("echo [mt7615e_up l1 profile] enter d8021xd_chk >> /tmp/wifiLuaDebug.log")
             d8021xd_chk(devname, dev.ext_ifname, dev.main_ifname, true)
        end
    elseif mtkwifi.exists("/etc/wireless/mt7615e/"..devname..".dat") then
        for _, pre in ipairs(vif_prefix) do
            ...
            if wifi_services_exist  then
                os.execute("echo [mt7615e_up dat] enter d8021xd_chk >> /tmp/wifiLuaDebug.log")
...
function mt7615e_down(devname)
    ...
    if l1parser and devs then
        ...
        if wifi_services_exist then
            os.execute("echo [mt7615e_down l1 profile] enter d8021xd_chk >> /tmp/wifiLuaDebug.log")
    ...
    elseif mtkwifi.exists("/etc/wireless/mt7615e/"..devname..".dat") then
        for _, pre in ipairs(vif_prefix) do
            if wifi_services_exist then
                os.execute("echo [mt7615e_down dat] enter d8021xd_chk >> /tmp/wifiLuaDebug.log")
...
function mt7615e_reload(devname)
    ...
    if not devname then
        ...
        --end
        if not mtkwifi.exists("/tmp/mtk/wifi/normal_reload") then
            os.execute("echo [mt7615e_reload] not exists /tmp/mtk/wifi/normal_reload file >> /tmp/wifiLuaDebug.log")
            _G["doBrctlAddIf"](0)
        else
        ...
        for devname, dev in mtkwifi.__spairs(devs.devname_ridx) do
            if qsetting then
                ...
                os.execute("echo [mt7615e_reload] path = "..path.." >> /tmp/wifiLuaDebug.log")
                if not mtkwifi.exists("/tmp/mtk/wifi/"..string.match(path, "([^/]+)\.dat")..".applied") then
                    ...
                    os.execute("echo [mt7615e_reload] not exist xxx.applied file, and creat xxx.last file >> /tmp/wifiLuaDebug.log") 
                else
                    ...
                    os.execute("echo [mt7615e_reload] devname = "..devname.." path = "..path.." >> /tmp/wifiLuaDebug.log")
                    if normal_reload then
                        os.execute("echo [mt7615e_reload] normal_reload is true >> /tmp/wifiLuaDebug.log")
                        break
                    end
                end
            end
        ...
        if normal_reload then
            os.execute("touch /tmp/mtk/wifi/normal_reload")
            for devname, dev in mtkwifi.__spairs(devs.devname_ridx) do
                mt7615e_down(devname)
            end
